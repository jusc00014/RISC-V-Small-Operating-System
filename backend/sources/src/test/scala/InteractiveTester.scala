...
class InteractiveTester extends AnyFlatSpec with ChiselScalatestTester with Matchers {
    ...
    def loadProgram(name: String, path: String, base: BigInt, config: Config): Program = {
        ...
        var labels = Map[String, String](
            ...
            "display_ready" -> config.display_ready.toString(16), 
            "display_data" -> config.display_data.toString(16))
    ...
    def loadInitialState(dut: MachineModeCoreTop, config: Config): InitialState = {
        return new InitialState(
            ...
            getCSRValues(dut),
            config.initial_memory + (config.mtime -> BigInt(10)),
            config.initial_pc
        )
    }
    ...
    def processInterrupts(dut: MachineModeCoreTop, state: State, config: Config) = {
        if (state.getMemoryValue(config.mtime, true) == state.getMemoryValue(config.mtimecmp, true) && state.getMemoryValue(config.mtimeh, true) == state.getMemoryValue(config.mtimecmph, true)) {
            dut.io_interrupt.m_timer.poke(true.B)
        } else {
            dut.io_interrupt.m_timer.poke(false.B)
        }
        dut.io_interrupt.m_ext.poke(false.B)
        if (dut.io_data.data_req.peekBoolean() && dut.io_data.data_we.peekBoolean() && dut.io_data.data_addr.peekInt() == config.display_data && state.getMemoryValue(config.display_ready) == 1) {
            state.addDisplayOutput(dut.io_data.data_wdata.peekInt().toChar)
            state.display_ready_timer = config.display_delay + 1
            state.addMemoryUpdate(Map(config.display_ready -> new ValueUpdate(state.getMemoryValue(config.display_ready), 0)))
        } else if (state.display_ready_timer > 1) {
            state.display_ready_timer -= 1
        } else if (state.display_ready_timer == 1) {
            state.addMemoryUpdate(Map(config.display_ready -> new ValueUpdate(state.getMemoryValue(config.display_ready), 1)))
            dut.io_interrupt.m_ext.poke(true.B)
            state.display_ready_timer = 0
        }
                ...
                case 1 => {
                    ...
                    for (i <- 0 until 3) {
                        if ((mask & (1 << i)) != 0) {
                            ...
                            data_1 |= state.getMemoryValue(address_1).toLong & (0xFF << (i * 8))
                        }
                    }
    ...
    def computeUpdate(dut: MachineModeCoreTop, state: State) = {
        ...
        state.addUpdate(new StateUpdate(state.next_index, pc, Map(reg_address -> new ValueUpdate(state.getRegisterValue(reg_address), reg_value)), computeCSRUpdate(dut, state), Map(), new ValueUpdate(state.getDisplayOutput(), state.getDisplayOutput())))
    }

    def computeCSRUpdate(dut: MachineModeCoreTop, state: State): Map[BigInt, ValueUpdate[BigInt]] = {
        return CSR_MAPPING.all.map(csr => {
            val old_value = dut.csr_rvfi(csr).rdata.peekInt() & dut.csr_rvfi(csr).rmask.peekInt()
    ...
    def resetCore(dut: MachineModeCoreTop, config: Config, frontend: TestFrontend): State = {
        ...
        state.display_ready_timer = config.display_delay + 1
        //state.addMemoryUpdate(Map(config.keyboard_ready -> new ValueUpdate(state.getMemoryValue(config.keyboard_ready), 0), config.display_ready -> new ValueUpdate(state.getMemoryValue(config.display_ready), 1)))
    ...
    it should "do an interactive test with Verilog" in {
        val frontend = new UIFrontend()
